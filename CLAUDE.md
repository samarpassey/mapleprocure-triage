...
routes the result with deterministic rules, notifies on strong matches and queues the rest for a
human. Postgres holds triage state and the audit trail. n8n 2.38.7 and Postgres 18 in Docker
Compose; Code node logic as plain JavaScript modules; the labelling CLI in stdlib Python.

The design is `MapleProcure Triage.pdf` at the repo root. It is not edited. Where this repo
departs from it, the departure is in `docs/DESIGN-AMENDMENTS.md`, keyed by the PDF's section.
...
11. **Secrets live only in `.env`.** The MapleProcure token is `read` only; nothing here needs or
    asks for `pii` or `export`.
...
| `docs/DESIGN-AMENDMENTS.md` | Before citing the PDF as the spec for states, routing or metrics. |
